<template lang="pug">
view.search-bar-box.class.flex
  aimg(:src="searchIcon")
  input.grow.m-l-15(:placeholder="placeholder" placeholder-class="placeholder" @input="methods._input" :value="data.value" confirm-type="search" @confirm="methods._confirm")
  view.mod.clear(@click="methods._clear" v-if="clearBtn && data.show.clearBtn")
    aimg(:src="clearIcon")
</template>
<script setup>
import {reactive} from 'vue'
//import {home} from '/common/config/url.json'
//import {__get} from '/common/script/req.promise'
//import {_tip, _go} from '/common/script/tip'
//import {onLoad} from '@dcloudio/uni-app'

const emits = defineEmits(['update:value', 'search'])
const props = defineProps({
  clearIcon: {type: String, default: 'data:image/svg+xml;base64,'},
  searchIcon: {type: String, default: 'data:image/svg+xml;base64,'},
  clearBtn: {type: Boolean, default: true},
  searchBtn: {type: Boolean, default: false},
  searchBtnText: {type: String, default: '搜索'},
  placeholder: {type: String, default: '请输入搜索内容'},
  value: {type: String, default: ''}
})

const data = reactive({
  value: '',
  show: {clearBtn: false}
})

const methods = {
  _input(e){
    const {value} = e.detail
    data.value = value
    emits('update:value', value)
    data.show.clearBtn = value.length > 0
    if(!value.length) methods._confirm()
  },
  _confirm(){
    emits('search', data.value)
  },
  _clear(){
    methods._input({detail: {value: ''}})
    methods._confirm()
  }
}

</script>
<script>
export default {// 微信小程序组件默认设置
  options: {
    styleIsolation: 'apply-shared',
    virtualHost: true
  },
  externalClasses: ['class'],
}
</script>
<style lang="scss" scoped>
@import '/common/scss/mixin.scss';
.search-bar-box{
  --aimg-width: 36rpx;
  margin: var(--search-bar-margin-top, 0) var(--search-bar-margin-right, 25rpx) var(--search-bar-margin-bottom, 0) var(--search-bar-margin-left, 25rpx);
  @include bg(var(--search-bar-bgcolor, #F8F8F8));
  padding: var(--search-bar-padding, 0 15rpx);
  height: var(--search-bar-height, 80rpx);
  border-radius: var(--search-bar-radius, 5rpx);
  .clear{
    margin-right: -25rpx;
  }
}
</style>